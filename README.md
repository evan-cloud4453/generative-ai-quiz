...
그래서 필요했던 건 예쁜 요약본이 아니라 **"틀린 것만 다시 물어봐 주는 기계"** 였습니다.
이 학습기의 설계는 전부 거기서 나왔습니다.

- 문제를 풀면 **틀린 문항이 자동으로 오답 노트에 쌓이고**,
- 나중에 그 문제를 맞히면 **오답 노트에서 알아서 빠집니다.**
- 그래서 시험 전날에는 "오답 노트만 다시 풀기" 버튼 하나만 계속 누르면 됩니다.
...
2. **그다음에는 오답 노트만** — 시험 전날까지 `오답 노트만 다시 풀기`를 반복합니다.
   오답 노트 문항 수가 0이 되면 한 바퀴 끝난 것입니다.
...
파일 하나로 끝나도록 만든 것은 의도한 선택입니다. 시험 기간에 환경 설정으로 시간을
쓰고 싶지 않았고, 링크만 있으면 어느 기기에서든 바로 열려야 했기 때문입니다.
...
그래서 변형 문항은 정답 하나만 다른 게 아니라, **네 개의 보기가 모두 그럴듯한 설명문**이
되도록 만들었습니다. 예를 들어 "VAE와 GAN의 차이"를 물을 때 오답 보기에도 실제로 존재하는
개념(적대적 학습, 확률 분포 근사, 노이즈 제거)을 섞어 두어, 개념을 헷갈리고 있으면
반드시 걸리도록 했습니다.
...
2026학년도 1학기 「생성형 인공지능 입문」 시험 대비로 만든 개인 학습용 프로젝트입니다.<br>
오답 노트가 0이 되는 순간까지 화이팅. ⭐
